...
<body class="bg-gray-50 font-sans text-gray-800">
    ...
    <main class="container mx-auto mt-6 px-4 pb-12">
        ...
        @if ($activeOrder)
            <div class="mb-8 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div class="p-4 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                    ...
                    <a href="{{ route('order.detail', $activeOrder->id) }}" class="text-sm text-indigo-600 font-semibold hover:text-indigo-800">
                    ...
                    @if ($activeOrder->session_status == 'locked')
                        <div class="flex items-start bg-red-50 border border-red-200 rounded-lg p-4 animate-pulse">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-red-700 mt-1">Mitra sedang login dan mengerjakan akun Anda saat ini. Membuka game dapat memicu sistem Banned dari developer.</p>
                            </div>
                        </div>
                    @else
                        <div class="flex items-start bg-green-50 border border-green-200 rounded-lg p-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-green-700 mt-1">Sesi pengerjaan sedang dihentikan sementara. Anda diizinkan untuk membuka game saat ini.</p>
                            </div>
                        </div>
                    @endif
                </div>
        ...
        <div class="flex space-x-2 mb-6 overflow-x-auto pb-2">
            <button id="all-filter" class="px-5 py-2 bg-indigo-600 text-white rounded-full text-sm font-medium whitespace-nowrap shadow-sm">Semua Jasa</button>
            <button id="single-filter" class="px-5 py-2 bg-white text-gray-600 border border-gray-200 rounded-full text-sm font-medium whitespace-nowrap transition-colors">Game Single-Player (Gacha)</button>
            <button id="comp-filter" class="px-5 py-2 bg-white text-gray-600 border border-gray-200 rounded-full text-sm font-medium whitespace-nowrap transition-colors">Game Kompetitif (Mabar)</button>
        </div>

        <h2 class="text-xl font-bold text-gray-900 mb-6">Rekomendasi Mitra Joki In</h2>

        <!-- Pesan Kosong untuk JavaScript (Pencarian / Filter) -->
        <div id="empty-state" class="hidden bg-white border border-gray-200 rounded-xl p-12 text-center shadow-sm">
            ...
            <h3 class="text-lg font-bold text-gray-700">Jasa Tidak Ditemukan</h3>
            <p class="text-gray-500 text-sm mt-1">Maaf, tidak ada layanan joki yang cocok dengan kriteria pencarian atau filter Anda.</p>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            @forelse ($services as $service)
                <div class="service-card bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow duration-300 flex flex-col"
                    ...
                    data-search="{{ Str::lower(e($service->game . ' ' . $service->title . ' ' . $service->mitra_name)) }}">

                    <div class="h-32 bg-gradient-to-r from-indigo-500 to-purple-600 flex items-center justify-center text-white relative">
                        <span class="font-bold text-lg opacity-80">{{ $service->game }}</span>
                        <span class="absolute top-2 right-2 bg-white text-gray-800 text-xs font-bold px-2 py-1 rounded shadow-sm">
                            {{ $service->category }}
                        </span>
                    </div>
                    ...
                    <div class="p-4 flex-1 flex flex-col">
                        ...
                        <h3 class="font-bold text-gray-900 text-sm mb-2 leading-tight flex-1">
                            {{ $service->title }}
                        </h3>
                        ...
                        <div class="mt-4 flex items-center justify-between border-t border-gray-100 pt-3">
                            <div>
                                ...
                                <span class="font-bold text-indigo-600">Rp {{ number_format($service->price, 0, ',', '.') }}</span>
                            </div>
                            <a href="{{ route('client.checkout', $service->id) }}" class="bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors">
                                Pesan
                            </a>
                        </div>
                    </div>
            ...
            @empty
                ...
                <div class="col-span-full bg-white border border-gray-200 rounded-xl p-12 text-center shadow-sm">
                    {{-- Di bawah ini kodenya diubah agar menjauhkan teks berpotensi @ dari compiler blade --}}
                    <svg class="w-16 h-16 text-indigo-500 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 11H5 m14 0 a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v6a2 2 0 012-2 m14-8V9a2 2 0 00-2-2H5a2 2 0 00-2 2v2"></path>
                    </svg>
                    <h3 class="text-lg font-bold text-gray-700">Belum Ada Layanan Tersedia</h3>
                    <p class="text-gray-500 text-sm mt-1">Saat ini mitra kami belum membuka slot pelayanan. Silakan cek kembali nanti!</p>
                </div>
            @endforelse
        </div>
    </main>
    ...
    <script>
        ...
        singleBtn.addEventListener('click', () => {
            currentCategory = 'Single-Player';
            setActiveButton(singleBtn);
            filterServices();
        });

        compBtn.addEventListener('click', () => {
            currentCategory = 'Kompetitif';
